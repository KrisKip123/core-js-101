...
function getPolynom(...arr) {
  return (x) => arr
    .reduceRight((sum, current, index) => sum + current * x ** (arr.length - 1 - index), 0);
}
...
function memoize(func) {
  const data = {};
  return (x) => {
    if (!Object.prototype.hasOwnProperty.call(data, func)) {
      data[func] = func(x);
    }
    return data[func];
  };
